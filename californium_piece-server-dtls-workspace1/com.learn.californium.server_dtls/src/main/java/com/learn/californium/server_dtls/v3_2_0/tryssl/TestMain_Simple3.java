...
public class TestMain_Simple3 {
	...
	public void my_configureToPrepare() {
		...
		if (CipherSuite.containsCipherSuiteRequiringCertExchange(suites)) {
			args.add("-cert");
			args.add(serverPemCertificate_loc);
		...
		String message = "Hello Scandium! hello";
		...
		for (int i=0;i<=1000-1;i++) {
			...
			if (process != null) {
				...
				try {
					 ...
					 * process.getOutputStream().flush(); try { Thread.sleep(1000); } catch
					 * (InterruptedException e) { // TODO Auto-generated catch block
					 * e.printStackTrace(); }
					 */
					System.out.println("< " + message2);
					process.getOutputStream().write(message2.getBytes());
