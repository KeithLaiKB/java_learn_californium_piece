...
public class CredentialsUtil {
	...
	//public static final List<Mode> DEFAULT_SERVER_MODES = Arrays.asList(Mode.PSK, Mode.ECDHE_PSK, Mode.RPK, Mode.X509, Mode.WANT_AUTH);
	//public static final List<Mode> DEFAULT_SERVER_MODES = Arrays.asList(Mode.PSK, Mode.ECDHE_PSK, Mode.RPK, Mode.X509, Mode.NO_AUTH);
	...
	// from demo-certs
	// client 私钥的 alias 在这个client中是不需要的，只是官方 server和client共用同一个文件, 而且我自己也分别改了他们的 密码
	public static final String CLIENT_NAME = "myclientakeystorealias";	//client 才会用到
	//
	//
	// 服务器的公钥的 alias和密码
	...
	// 服务器的私钥的 alias和密码
	public static final String SERVER_NAME = "mykeystorealias";			//server 才会用到
	...
	public static List<Mode> parse(String[] args, List<Mode> defaults, List<Mode> supported) {
		...
		//
		if (defaults != null) {
			// 如果default的mode内容列表时空的, 则把所有的都加进去
			if (modes.isEmpty()
	...
	public static void setupCredentials(DtlsConnectorConfig.Builder config, String certificateAlias, List<Mode> modes) {
		...
		if (x509 >= 0 || rpk >= 0) {
			try {
				...
				//SslContextUtil.Credentials serverCredentials = SslContextUtil.loadCredentials(SslContextUtil.CLASSPATH_SCHEME + KEY_STORE_LOCATION, certificateAlias, KEY_STORE_PASSWORD,KEY_STORE_PASSWORD);
				
				String myusr_path = System.getProperty("user.dir");
				SslContextUtil.Credentials serverCredentials = SslContextUtil.loadCredentials(
						myusr_path + "\\" + KEY_STORE_LOCATION, certificateAlias, KEY_STORE_PASSWORD,
						KEY_STORE_PASSWORD);
				if (!noAuth) {
					if (x509 >= 0) {
						//Certificate[] trustedCertificates = SslContextUtil.loadTrustedCertificates(SslContextUtil.CLASSPATH_SCHEME + TRUST_STORE_LOCATION, TRUST_NAME,TRUST_STORE_PASSWORD);
						Certificate[] trustedCertificates = SslContextUtil.loadTrustedCertificates(
								myusr_path + "\\" + TRUST_STORE_LOCATION, TRUST_NAME, TRUST_STORE_PASSWORD);
